  ...
  static Future<void> savePushNotification(RemoteMessage message) async {
    try {
      ...
      final notifMap = {
        ...
        'category': message.data['category'] ?? message.data['type'],
        'target_id': message.data['target_id'] ?? message.data['event_id'] ?? message.data['team_id'] ?? message.data['lost_found_id'] ?? message.data['lostfound_id'],
        'post_image': message.data['post_image'] ?? message.data['image'] ?? message.data['photo'],
        'created_at': DateTime.now().toIso8601String(),
      };
    ...
    try {
      ...
      return listStr.map((e) {
        ...
        return AnnouncementModel(
          ...
          targetId: map['target_id']?.toString() ?? map['event_id']?.toString() ?? map['team_id']?.toString() ?? map['lost_found_id']?.toString(),
          postImage: map['post_image'] ?? map['image'] ?? map['photo'],
